...
class V2rayClientApp(customtkinter.CTk):
    ...
    def _setup_tray_icon(self):
        """设置系统托盘图标和右键菜单"""
        icon_path = resource_path(os.path.join('v2fly-core', 'v2ray.png'))
        try:
            image = Image.open(icon_path)
        except FileNotFoundError:
            self.log_message(f"错误: 找不到图标文件: {icon_path}，将使用默认图标。" )
            # 如果找不到图标文件，就创建一个黑底白圈的默认图标
            image = Image.new('RGB', (64, 64), 'black')
            draw = ImageDraw.Draw(image)
            draw.ellipse((10, 10, 54, 54), fill='white')
        except Exception as e:
            self.log_message(f"加载图标文件 {icon_path} 失败: {e}，将使用默认图标。" )
            image = Image.new('RGB', (64, 64), 'black')
            draw = ImageDraw.Draw(image)
            draw.ellipse((10, 10, 54, 54), fill='white')
